...
`Input` 内部通过 hooks 维护一个稳定的 `TextEditingController`，因此必须用 `<Input … />` 以元素形式挂载，不能直接当函数调用——直接调用会以「hooks may only run in a function component」失败。编辑细节见[文本编辑指南](/guide/editing)。
