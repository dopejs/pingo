...
שאינן חלק מהחוזה הציבורי — [סורק ההגירה](/guide/migration) ידחה ייבוא ישיר שלהן.
...
function Counter() {
  ...
  return <text value={`עברו ${count} שניות`} />;
}
```
...
אלו הם הנתונים הראשוניים לאיתור בעיות ביצועים. למידע נוסף ראו [אבחון](/guide/diagnostics).
