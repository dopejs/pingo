...
| WASM 体积（gzip，base 档位）                 | < 400 KiB（409,600 bytes）                |
| WASM 体积（gzip，rich-text 档位）            | < 440 KiB（450,560 bytes）                |
| 各档位工程余量（gzip）                       | 产品硬上限下保留至少 16 KiB               |
...
能力），但**不在默认发布产物里**：编译进来约 19 KB gzip，不进 `base` 档位。
`PINGO_RICH_TEXT=1 pnpm core:wasm` 写出 `rich-text` 档位的产物，按该档位自己的预算把关。

**两个档位，两个预算。** 一个只做文本渲染的引擎和一个还带文档编辑器的引擎不是同一个
下载物，用同一个数字约束两者，会把「新增能力」变成「顶破门禁」。`base` 保持 400 KiB
不动——不要编辑器的应用不为编辑器付费；`rich-text` 有自己的 440 KiB，余量按 E15 还欠的
工作（点击定位光标、文档 IME、撤销重做键位）留出，免得做到一半重谈口径。两个档位的构建
都按「产品硬上限下 16 KiB」的工程门禁把关，口径一致。

当前：`base` 389,340（工程门禁下余 3,876），`rich-text` 408,827（工程门禁下余 25,349）。
归因见 [`wasm-size-attribution.md`](wasm-size-attribution.md)。
...
- **WASM 体积**进卡点，按**发布档位**分别计量（§2 表）：`base` < 400 KiB，
  `rich-text` < 440 KiB。构建按各自的工程门禁（产品硬上限下 16 KiB 余量）把关，
  产品硬上限由发布候选检查。
