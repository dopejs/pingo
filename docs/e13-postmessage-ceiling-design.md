...
1. **去掉每次输入的 `.slice()`**：改用池化缓冲或环形复用，减少每帧的分配与 GC 压力。
2. **小载荷不 transfer**：transfer 会 detach/reattach ArrayBuffer，对几十字节的输入
   可能比结构化克隆更贵。需要按载荷大小实测取舍。
...
**前四条都不改 ABI，也不改降级链语义**，属于 transport 实现内部。
