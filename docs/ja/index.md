...
P0–M8 の全エンジニアリングマイルストーンが完了。M9「本番適格性、増分合成、リリース強化」は計画済みで未着手。
詳細は [M9 プラン](/m9-production-plan) を参照。現在のリポジトリ変更は依然として Unreleased であり、新しい npm バージョンが公開されたことを意味しない。

実機性能、実入力メソッド、スクリーンリーダー、メディア消費電力マトリクスはプラットフォーム適格性の収集項目として個別に追跡し、
bidi ビジュアルナビゲーションと WebGPU バックエンドのデフォルト有効化は[記録済みの延期項目](/plan)である。
