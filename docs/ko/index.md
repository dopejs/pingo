...
P0–M8의 모든 엔지니어링 마일스톤이 완료되었습니다. M9 "프로덕션 자격, 증분 합성 및 릴리스 강화"는 계획이 완료되었으나 구현은 아직 시작되지 않았습니다. 자세한 내용은 [M9 계획](/m9-production-plan)을 참조하십시오. 현재 저장소의 변경 사항은 아직 Unreleased 상태이며, 새로운 npm 버전이 게시되었음을 의미하지 않습니다.

실기기 성능, 실제 입력기, 스크린 리더, 미디어 전력 소비 매트릭스는 플랫폼 자격 수집 항목으로 별도 추적됩니다. bidi 시각 내비게이션과 WebGPU 백엔드 기본 활성화는 여전히 [기록된 연기 항목](/plan)입니다.
