...
> 状态：初版（2026-08-17）。发布集共 14 个包：组件库 `@dopejs/pingo-ui`、公开入口 `@dopejs/pingo`、
> 迁移边界 `@dopejs/pingo-compat`、React 绑定 `@dopejs/pingo-react`，以及它们的依赖闭包（runtime/editing/style/
...
- 全部 14 个包**同版本**发布；`ENGINE_VERSION`（facade 导出）与包版本
...
4. 创建 GitHub Release：自动生成 release notes，附上 14 个包的 tarball 与
...
```sh
# 1. 设定版本（同步 14 个包 + ENGINE_VERSION）
