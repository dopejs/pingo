...
- `@dopejs/pingo-ui` 開始發佈到 npm，與引擎同版本。它此前只存在於倉庫與文件站，
  發佈集裡沒有它，因此 46 個元件有文件卻裝不到。納入發佈集也同時開啟了它的產物
  驗證：必需檔案、授權檔案、workspace 相依改寫與相依閉包。
...
- 公開冪等且可重試的 `initializeWasm`，業務可以自行編排 WASM 載入；預設 Storybook
  loading 改為輕量延遲顯示，Worker 初始化沿用同一入口。
- 雙時鐘 Playground 改為進入頁面即持續的百萬列虛擬捲動；按鈕只阻塞主執行緒，不再
  啟動或重設捲動狀態。
- 新增由 Core/Worker 時鐘持有的等速程式化捲動 `setScrollVelocity`；Input Stream
  增加對應指令，ABI 版本 10 → 11。
